...
output: 
  flexdashboard::flex_dashboard:
    ...
    css: custom.css
vignette: >
  %\VignetteIndexEntry{StreamLight_tutorial}
...
Running the **StreamLight** model requires a parameter file that describes various site characteristics and a driver file that contains inputs into the model. For convenience, a series of functions have been included in the companion package **StreamLightUtils** to derive some of the values required for parameter files and 
to create standardized driver files for use with **StreamLight**. Remotely sensed data products with good broadscale coverage are used to derive these inputs within **StreamLightUtils**, but there are of course many potential sources for similar data that could be used to create driver files and users are welcome to create their own workflow for creating driver or parameter files. This tutorial uses two sites from North Carolina to work through using **StreamLightUtils** to create standardized model driver files, creating parameter files, and generating estimtes using **StreamLight**. An outline of topics covered in this tutorial are presented below:

**<font size="+2">Outline</font>**

1.  **Overview and getting started**

    1.1  <span style="color:black">Overview of the </span> **<span style="color:DarkOrange">stream_light</span>** function
    
    1.2  Getting started
    
2.  **Preparing a driver file**

    2.1  <span style="color:black">Downloading and processing total incoming shortwave radiation</span>
    
    2.2  <span style="color:black">Downloading and processing MODIS LAI data</span>
    
    2.3  <span style="color:black">Creating a standardized driver file</span>

3.  **Preparing site parameters**

    3.1  <span style="color:black">Parameter descriptions and values</span>

4.  **Running the StreamLight model**

    4.1  <span style="color:black">Running the model</span>
    
    4.2  <span style="color:black">Aggregating hourly estimates to daily</span>
    
    
**<font size="+2">1.1 Overview of the <span style="color:DarkOrange">stream_light</span> function</font>**
...
|        incoming irradiance (W m^-2^) and leaf area index (LAI) (m^2^ m^-2^) which are used as model inputs. The remaining arguments in the function are
|        parameters that describe site characteristics. On the surface this seems like a large number of parameters;however, section 3 of this tutorial 
|        provides more indepth information on each of these parameters and some simplifying assumptions that can be used to reduce the number of 
|        necessary parameters.


**<font size="+2">1.2 Getting started</font>**
...
#Load the StreamLightUtils library
...
2. Preparing driver files
================================

**<font size="+4">Preparing a driver file</font>**
...
**<font size="+2">2.1 Downloading and processing total incoming shortwave radiation (W m^-2^)</font>**


>Total incoming shortwave radiation (W m^-2^) is provided by the National Land Data Assimilation System (NLDAS) at hourly timestpes at 0.125 degree spatial resolution. There are two potential workflows available: i.) download data for a single site, and ii.) bulk download of data for many sites.

i.  **Downloading and processing NLDAS data for a single site**
    ...
    *   *<span style="color:#009faf">startDate</span>*  The start date for the downloaded data ("YYYY-MM-DD")
    We can use site information for NC_NHC to download data at this site.  
...
working_dir <- "C:/research/StreamLight vignette"
```

ii.  **Downloading and processing NLDAS data for multiple sites**
      ...
      *   *<span style="color:#009faf">startDate</span>*  An optional parameter. By default, if nothing is provided the function assumes that site_locs has a column that contains startDate. Alternatively, a single startDate can be provided as an argument for the download (YYYY-MM-DD)
      ...
      Recall from earlier that our table of site information has a column called "startDate", so here the optional parameter is not used.
    
      If data fails to download for a site, the **<span style="color:DarkOrange">NLDAS_DL_bulk</span>** function will automatically check and retry downloading data for all sites with  missing data. However, it is possible that data does not exist for a site. Consequently, it is prudent to confirm the successfully downloaded sites.
```{r echo=TRUE, results='hide', eval = FALSE}
...
**<font size="+2">2.2 Downloading and processing MODIS LAI data</font>**
...
i.  **Export a table of site information**
...
ii.  **Submitting a request**
      ...
      * The NC_sites.csv that was exported can now simply be drag-and-dropped to populate a list of sites. You may notice that special characters like "_" are removed from the ID field, don't worry this is ok. 
      ...
      <center>
      ![Caption for the picture.](C:/research/StreamLight vignette/images/drag and drop.png)
      </center>
    
      <center>
      ![](C:/research/StreamLight vignette/images/drag and drop results.png)
      </center>
      ...
      * Type MCD15A3H into the layers search box and select the product. Add all of the layers to the selected layers.
      ...
      <center>
      ![](C:/research/StreamLight vignette/images/filled request submission.png)
      </center>
...
iii.  **Unpacking the downloaded LAI data**
...
iv.  **Processing the downloaded LAI data**
...
```    
  ...
  <center>
  ![](C:/research/StreamLight vignette/images/NC_NHC_LAI_plot.png)
  </center>
      
  <center>
  ![](C:/research/StreamLight vignette/images/NC_UEno_LAI_plot.png)
  </center>
      
      
**<font size="+2">2.3 Creating a standardized driver file</font>**
...
|               $\circ$  *<span style="color:#009faf">site_locs</span>* A table with Site_ID, Lat, and Lon, and the coordinate reference system designated as an EPSG code. 
|                For example, the most common geographic system is WGS84 and its EPSG code is 4326
|               $\circ$  *<span style="color:#009faf">NLDAS_processed</span>* Output from the **<span style="color:DarkOrange">NLDAS_proc</span>** function
|               $\circ$  *<span style="color:#009faf">MOD_processed</span>* Output from the **<span style="color:DarkOrange">AppEEARS_proc</span>** function
|               $\circ$  *<span style="color:#009faf">write_output</span>* Logical value to indicate whether to write each individual driver file to disk. Default value is FALSE.
|               $\circ$  *<span style="color:#009faf">save_dir</span>* Optional parameter when write_output = TRUE. The save directory for files to be placed in. For example, "C:/
|
|
...
|               $\circ$  "local_time" A POSIXct object in local time
|               $\circ$  "offset" The offset of local time from UTC, used in the solar_c function
|               $\circ$  "jday" A unique identifier for each day in the format YYYY + day of year
|               $\circ$  "DOY" The day of year (1-365 or 366 for leap years)
|               $\circ$  "Hour" Hour (0-23)
|               $\circ$  "SW_inc" Total incoming shortwave radiation (W m^-2^) from NLDAS 
|               $\circ$  "LAI" MODIS leaf area index (m^2^ m^-2^) interpolated to daily values by **<span style="color:DarkOrange">AppEEARS_proc</span>** 

3. Preparing site parameters
================================

**<font size="+4">Preparing a parameter file</font>**
...
There are several site parameters required to run **StreamLight**; however, not all of these parameters have built in functions within **StreamLightUtils**. Similarly, not all parameters are easily obtained nor will they all have equal importance for model performance. Here, we detail the same process used to extract parameter values from Savoy et al. (in review). To begin with, let's revisit the parameters used:
  ...
  * A schematic of various input parameters.
    ...
    <center>
    ![](C:/research/StreamLight vignette/images/diagram.png)
    </center>
...
i.  **Channel azimuth (*<span style="color:#009faf">channel_azimuth</span>*)**
    ...
    * Example of deriving azimuth, note the first azimuth of the first example is 45$^\circ$ whereas the second example is 315$^\circ$.
      ...
      <center>
      ![](C:/research/StreamLight vignette/images/measuring stream azimuth.png)  
      </center>
      
ii.  **Width (*<span style="color:#009faf">bottom_width</span>*)**
...
vii.  **Maximum canopy overhang (*<span style="color:#009faf">overhang</span>*)**

      Without detailed information on canopy overhang it was assumed that overhang was 10% of tree height at all sites.

viii.  **Height of maximum canopy overhang (*<span style="color:#009faf">overhang_height</span>*)**
...
ix.  **Leaf angle distribution(*<span style="color:#009faf">x_LAD</span>*)**
...
**<font size="+4">Running StreamLight</font>**

First time installation of the **StreamLight** package from GitHub can be done with the devtools library.

devtools::install_github("psavoy/StreamLight")
 
Once installed, the package can be loaded as normal.
```{r, echo=TRUE}
library("StreamLight")
```
...
**<font size="+2">4.1 Running the model</font>**

i.  **Generate estimates for a single site**
...
ii.  **Generate estimates for multiple sites**
...
|               $\circ$  "SW_inc" Total incoming shortwave radiation (W m^-2^) from NLDAS 
|               $\circ$  "LAI" MODIS leaf area index (m^2^ m^-2^) interpolated to daily values by **<span style="color:DarkOrange">AppEEARS_proc</span>** 
|               $\circ$  "PAR_inc" -- Incoming PAR above the canopy (<font size="+1">$\mu$</font>mol m^-2^ s^-1^) 
|               $\circ$  "PAR_bc" -- Estimated PAR (<font size="+1">$\mu$</font>mol m^-2^ s^-1^) directly below the canopy 
|               $\circ$  "veg_shade" The proportion of the channel crossection that is shaded by riparian vegetation
|               $\circ$  "bank_shade" -- The proportion of the channel crossection that is shaded by stream banks
|               $\circ$  "PAR_stream" The estimated PAR for the channel cross section (<font size="+1">$\mu$</font>mol m^-2^ s^-1^)
